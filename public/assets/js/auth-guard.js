...
(async function () {
    ...
    let canonicalPage = '';
    if (path.includes('/staff/admin/') || path.includes('admin_dashboard.html')) {
        canonicalPage = 'admin_dashboard.html';
    } else if (path.includes('/staff/hr/') || path.includes('hr_dashboard.html')) {
        canonicalPage = 'hr_dashboard.html';
    } else if (path.includes('/staff/programs/') || path.includes('programs_dashboard.html')) {
        canonicalPage = 'programs_dashboard.html';
    } else if (path.includes('/staff/communications/') || path.includes('communications_dashboard.html')) {
        canonicalPage = 'communications_dashboard.html';
    } else if (path.includes('/staff/me/') || path.includes('me_dashboard.html')) {
        canonicalPage = 'me_dashboard.html';
    } else if (path.includes('/staff/finance/') || path.includes('finance_dashboard.html')) {
        canonicalPage = 'finance_dashboard.html';
    } else if (path.endsWith('/staff/') || path.endsWith('/staff') || path.endsWith('staff.html')) {
        canonicalPage = 'staff.html';
    } else {
        canonicalPage = path.split('/').pop() || 'index.html';
    }

    const currentPage = canonicalPage;
    ...
    if (isProtected) {
        ...
        if (!isAdmin) {
            ...
            if (dashboardPages.includes(currentPage)) {
                const cleanPageMap = {
                    'admin_dashboard.html': '/staff/admin/',
                    'hr_dashboard.html': '/staff/hr/',
                    'programs_dashboard.html': '/staff/programs/',
                    'communications_dashboard.html': '/staff/communications/',
                    'me_dashboard.html': '/staff/me/',
                    'finance_dashboard.html': '/staff/finance/'
                };
                if (cleanPageMap[currentPage] !== cachedPrimary) {
    ...
    if (isProtected) {
        ...
        if (!isAdmin) {
            ...
            if (dashboardPages.includes(currentPage)) {
                ...
                if (cleanPageMap[currentPage] !== verifiedPrimary) {
                    sessionStorage.setItem('auth_alert', 'restricted');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (userId) {
            ...
            // E. Dynamic Sidebar Injection
            const asideElement = document.querySelector('aside:not(#admin-sidebar)') || document.getElementById('admin-sidebar-placeholder');
            if (asideElement) {
                const isAdmin = ['superadmin', 'admin'].includes(role) || designation.includes('executive') || designation.includes('director');
                
                // Inject responsive CSS stylesheet for mobile menu toggle support
                const respStyle = document.createElement('style');
                respStyle.textContent = `
                    @media (max-width: 767px) {
                        aside.mobile-active, #admin-sidebar.open {
                        ...
                        aside:not(.mobile-active):not(.open), #admin-sidebar:not(.open) {
                            transform: translateX(-100%) !important;
                            transition: transform 0.3s cubic-bezier(0.4, 0, 0.2, 1) !important;
                            display: none !important;
                        }
                        #mobile-sidebar-overlay, #admin-mobile-overlay {
                            position: fixed;
                            top: 0;
                            left: 0;
                            width: 100vw;
                        ...
                        #mobile-sidebar-overlay.active, #admin-mobile-overlay.active {
                ...
                // Create Overlay
                let overlay = document.getElementById('mobile-sidebar-overlay') || document.getElementById('admin-mobile-overlay');
                if (!overlay) {
                    overlay = document.createElement('div');
                    overlay.id = isAdmin ? 'admin-mobile-overlay' : 'mobile-sidebar-overlay';
                    overlay.className = isAdmin ? 'fixed inset-0 bg-black/50 z-30 hidden lg:hidden' : '';
                    document.body.appendChild(overlay);
                }

                if (isAdmin) {
                    // Fetch and inject admin-sidebar.html dynamically
                    fetch('/staff/admin-sidebar.html')
                        .then(res => {
                            if (!res.ok) throw new Error(`HTTP ${res.status}`);
                            return res.text();
                        })
                        .then(html => {
                            // If placeholder, replace it. If aside, replace it.
                            if (asideElement.outerHTML) {
                                asideElement.outerHTML = html;
                            } else {
                        ...
                        .catch(err => {
                            console.error('[AuthGuard] Failed to load admin sidebar:', err);
                        });
                        
                    // Define initialization helper inside block scope
                    function initAdminSidebar() {
                        const path = window.location.pathname;
                        const filename = path.split('/').pop() || 'admin_dashboard.html';
                        const urlParams = new URLSearchParams(window.location.search);
                        
                        // 1. Get active page ID
                        let pageId = 'admin_dashboard';
                        if (filename === 'cms_dashboard.html') {
                            const tab = urlParams.get('tab');
                            if (tab === 'pages') pageId = 'admin_pages';
                            else if (tab === 'team') pageId = 'admin_team';
                            else if (tab === 'stats') pageId = 'admin_stats';
                            else if (tab === 'gallery') pageId = 'cms_gallery';
                            else pageId = 'cms_dashboard';
                        } else if (path.includes('/staff/admin/') || filename === 'admin_dashboard.html') {
                            pageId = 'admin_dashboard';
                        } else if (path.includes('/staff/hr/') || filename === 'hr_dashboard.html') {
                            pageId = 'hr_dashboard';
                        } else if (path.includes('/staff/programs/') || filename === 'programs_dashboard.html') {
                            pageId = 'programs_dashboard';
                        } else if (path.includes('/staff/finance/') || filename === 'finance_dashboard.html') {
                            pageId = 'finance_dashboard';
                        } else if (path.includes('/staff/communications/') || filename === 'communications_dashboard.html') {
                            pageId = 'communications_dashboard';
                        } else if (path.includes('/staff/me/') || filename === 'me_dashboard.html') {
                            pageId = 'me_dashboard';
                        } else {
                            const pageMap = {
                            ...
                            pageId = pageMap[filename] || 'admin_dashboard';
                        }
                        ...
                        // 4. Setup mobile drawer
                        const sidebar = document.getElementById('admin-sidebar') || document.querySelector('aside.sidebar') || document.querySelector('aside');
                        const burgerOverlay = document.getElementById('admin-mobile-overlay') || document.getElementById('mobile-sidebar-overlay');
                        
                        // Dynamically create hamburger in header if not already present
                        const header = document.querySelector('header');
                        if (header) {
                            let flexContainer = header;
                            const innerFlex = header.querySelector('.flex, div[class*="flex"]');
                            if (innerFlex) flexContainer = innerFlex;
                            
                            let burgerBtn = document.getElementById('admin-hamburger-btn') || document.getElementById('mobile-hamburger-btn');
                            if (!burgerBtn) {
                                burgerBtn = document.createElement('button');
                                burgerBtn.id = 'admin-hamburger-btn';
                        ...
                        if (sidebar && burgerOverlay) {
                            ...
                            burgerOverlay.addEventListener('click', closeSidebar);
                        ...
                        // 5. Setup logout
                        const logoutBtn = document.getElementById('admin-logout-btn') || document.getElementById('logout-btn');
                        if (logoutBtn) {
                    ...
                    // Inject standard flat menu for non-admins
                    ...
                    if (primaryDashboard !== '/staff/staff.html') {
                        menuItems.push({ name: 'Department Dashboard', url: primaryDashboard, icon: 'dashboard' });
                    }
                    menuItems.push({ name: 'User Dashboard', url: '/staff/staff.html', icon: 'hub' });
                    menuItems.push(
                        { name: 'Team Directory', url: '/staff/team_directory.html', icon: 'group' },
                        { name: 'Impact Analytics', url: '/staff/impact_analytics.html', icon: 'bar_chart' },
                        { name: 'CMS Console', url: '/staff/cms_dashboard.html', icon: 'settings_suggest' },
                        { name: 'My Settings', url: '/staff/edit_profile.html', icon: 'settings' },
                        { name: 'Ecosystem Site', url: '/public/homepage.html', icon: 'home' }
                    );
                    ...
                    asideElement.innerHTML = `
                        <div class="flex flex-col h-full py-base">
                            <div class="px-6 py-6 border-none flex justify-between items-center">
                                <a href="/public/homepage.html" class="flex items-center gap-3">
                    ...
                    if (header) {
                        ...
                        if (!classes.includes('sticky') && !classes.includes('fixed')) classes.push('sticky');
                        ...
                        let burgerBtn = document.getElementById('mobile-hamburger-btn') || document.getElementById('admin-hamburger-btn');
                        if (!burgerBtn) {
                            burgerBtn = document.createElement('button');
